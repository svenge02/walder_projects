...
filialliste = ['098', '099', '101', '102', '103', '105', '107', '114', '117', '120',
               ...
               '233', '234', '240', '244', '246', '342', '408', '411', '780',
               '782', '793']

warengruppen = [0, 1, 2, 3, 4, 5, 6, 9]

spalten = ['BEKN', 'asw']
listen_werte = ['BMEN','BVKS', 'BVKA','BEKN', 'asw']
...
df_neu['SAISON'] = df_neu['SAISON'].str[:2]
# df_neu['SAISON'] = df_neu['SAISON'].str.split(" - ", n=1, expand=True) bringt neu Fehlermeldung
print(df_neu)
...
xw.sheets['aktuell'].range('c5').value = df_select.groupby(['Kennung'])[listen_werte].agg('sum')
xw.sheets['aktuell'].range('B16').value = df_wg0.groupby(['WAG', 'Kennung'])[listen_werte].agg('sum')
xw.sheets['aktuell'].range('B27').value = df_wg1.groupby(['WAG', 'Kennung'])[listen_werte].agg('sum')
xw.sheets['aktuell'].range('B38').value = df_wg2.groupby(['WAG', 'Kennung'])[listen_werte].agg('sum')
xw.sheets['aktuell'].range('B49').value = df_wg3.groupby(['WAG', 'Kennung'])[listen_werte].agg('sum')
xw.sheets['aktuell'].range('B60').value = df_wg4.groupby(['WAG', 'Kennung'])[listen_werte].agg('sum')
xw.sheets['aktuell'].range('B71').value = df_wg5.groupby(['WAG', 'Kennung'])[listen_werte].agg('sum')
xw.sheets['aktuell'].range('B82').value = df_wg6.groupby(['WAG', 'Kennung'])[listen_werte].agg('sum')
xw.sheets['aktuell'].range('B93').value = df_wg9.groupby(['WAG', 'Kennung'])[listen_werte].agg('sum')

xw.sheets['aktuell'].range('C105').value = df_select.groupby(['FILIALE'])[listen_werte].agg('sum')
xw.sheets['aktuell'].range('C149').value = df_un_select.groupby(['FILIALE'])[listen_werte].agg('sum')